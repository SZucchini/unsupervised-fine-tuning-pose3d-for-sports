...
def motionAGF_inferencer(cfg, detector, pose_estimator, pose_lifter):
    input_dirs = natsorted(glob.glob(cfg.INPUT_DIR + '/*'))
    if len(input_dirs) <= 3:
        video_dirs = [natsorted(glob.glob(cfg.INPUT_DIR + '/*.mp4'))]
    else:
        video_dirs = [natsorted(glob.glob(input_dir + '/*.mp4')) for input_dir in input_dirs]

    kpts3d_all, kpts2d_all, score3d_all, score2d_all = [], [], [], []
    for i, video_files in enumerate(video_dirs):
        parent_dir = os.path.dirname(video_files[0])
        logger.debug("Start processing video files in {}".format(parent_dir))
        ...
        for input_video in video_files:
            kpts2d, score2d, img_size = estimate2d(cfg, input_video, detector, pose_estimator)
            kpts3d, score3d = estimate3d(cfg, pose_lifter, kpts2d, score2d, img_size)

            kpts2d_per_data.append(kpts2d)
            kpts3d_per_data.append(kpts3d)
            score2d_per_data.append(score2d)
            score3d_per_data.append(score3d)

        kpts2d_per_data = np.array(kpts2d_per_data)
        kpts3d_per_data = np.array(kpts3d_per_data)
        score2d_per_data = np.array(score2d_per_data)
        score3d_per_data = np.array(score3d_per_data)

        logger.debug("Finish processing video files in {}".format(parent_dir))

        kpts2d_all.append(kpts2d_per_data)
        kpts3d_all.append(kpts3d_per_data)
        score2d_all.append(score2d_per_data)
        score3d_all.append(score3d_per_data)

    return kpts2d_all, score2d_all, kpts3d_all, score3d_all
